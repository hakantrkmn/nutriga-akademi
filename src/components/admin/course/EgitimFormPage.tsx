...
export default function EgitimFormPage({ egitimId }: { egitimId?: string }) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (isEditing.current && egitimId) {
        ...
        // Yeni oluştur
        response = await egitimlerApi.create(data as Omit<Egitim, 'id' | 'createdAt' | 'updatedAt'>)
      }

      if (response.success) {
        console.log(isEditing.current ? 'Eğitim güncellendi' : 'Eğitim oluşturuldu')
        router.push('/admin/egitimler')
      } else {
  ...
  return (
    <Box bg="gray.50" minH="100vh">
        ...
        <Container maxW="1200px" px={{ base: 4, md: 6 }} py={{ base: 4, md: 6 }}>
          <HStack justify="space-between" align="center">
            <Box>
              ...
              <Text color="gray.600">
                {isEditing.current ? 'Eğitim bilgilerini düzenleyin' : 'Yeni bir eğitim oluşturun'}
              </Text>
            </Box>
            <HStack gap={3}>
              <Button
      ...
      <EgitimDetailHero egitim={formData} isImageError={false} setIsImageError={() => {}} />
