...
export async function sendContactMessageNotificationToAdmin({
  userName,
  userEmail,
  phone,
  message,
  messageDate,
}: ContactMessageNotificationParams) {
  try {
    const transporter = createGmailTransporter();
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #2563eb; border-bottom: 2px solid #2563eb; padding-bottom: 10px;">
          📧 Yeni İletişim Mesajı - Nutriga Akademi
        </h2>

        <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
          <h3 style="color: #1e40af; margin-top: 0;">Gönderen Bilgileri</h3>
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr>
              <td style="padding: 8px 0; font-weight: bold; color: #374151;">Gönderim Tarihi:</td>
              <td style="padding: 8px 0; color: #6b7280;">${messageDate}</td>
            </tr>
          </table>
        </div>
        ...
        <div style="background-color: #fef3c7; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #f59e0b;">
          ...
          <p style="color: #78350f; line-height: 1.6; white-space: pre-wrap; margin: 0;">${message}</p>
        </div>

        <div style="background-color: #ecfdf5; padding: 15px; border-radius: 8px; border-left: 4px solid #10b981;">
          <p style="margin: 0; color: #065f46;">
            <strong>💡 Bilgi:</strong> Bu mesajı admin panelinden görüntüleyebilir ve yanıtlayabilirsiniz.
          </p>
        </div>
    ...
    const info = await transporter.sendMail({
      from: `"Nutriga Akademi" <${process.env.GMAIL_USER}>`,
      to: process.env.ADMIN_EMAIL, // Admin email adresiniz
      subject: `📧 Yeni İletişim Mesajı: ${userName}`,
      html: emailHtml,
    });

    console.log(
      "Gmail SMTP ile iletişim mesajı bildirimi gönderildi:",
      info.messageId
    );
    return { success: true, messageId: info.messageId };
  } catch (error) {
